...
class ttdController extends Controller
{
    ...
    {
        $data = TtdDataModel::select('*')->where('id', '=', $id)->get();
        if (count($data) > 0) {
    ...
    {
        $data = TtdDataModel::select('*')->where('unique_id', '=', $id)->get();
        if (count($data) > 0) {
            $dataSprin['data'] = $data[0];
    ...
    function kwitansiDokter()
    {
        $receipts = Receipt::all(); // Fetch all receipts from DB
        return view('kwitansi_dashboard', compact('receipts'));
    }
}
